...
登录页保存配置时，输入控件会提供与后端校验一致的轻量提示属性和隐藏辅助说明；真正的边界仍以后端返回的中文错误为准。已保存的敏感字段会显示脱敏占位符。留空规则：
...
- 代理只支持 `socks4://` 或 `socks5://`，host 必填，端口默认 1080 且必须在 `1..65535`，不能带 path、query 或 fragment；含凭据代理不会在登录页回显原文。
