...
- `api_id`、`api_hash`、手机号、SOCKS5 代理、Session 文件名、StringSession、下载线程、缓存上限、Web Token 和 `.session` 上传控件都有可理解标签。
...
- 搜索框说明可理解，可输入名称、用户名或 ID 过滤；清空搜索后列表恢复。
...
- 查看器打开后焦点进入关闭按钮。
- 查看器顶部显示媒体标题和当前索引。
- 关闭、下载、上一项和下一项按钮都有明确用途，图标按钮不会与媒体内容重叠。
